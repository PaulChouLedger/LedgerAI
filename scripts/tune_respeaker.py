...
Presets:
    - clean: HPF + Conservative AGC + Max Stationary NS - DEFAULT
...
def configure_clean():
    ...
    print("\n" + "="*80)
    print("  🧹 CONFIGURING CLEAN AUDIO (HPF + Conservative AGC + Stationary NS)")
    print("="*80 + "\n")
    ...
    # Enable conservative AGC for far-field pickup
    print("[2/4] Hardware AGC: ON (conservative - 0.08 RMS target)")
    dev.write("AGCONOFF", 1)
    dev.write("AGCDESIREDLEVEL", 0.08)  # Conservative target (not too loud)
    dev.write("AGCMAXGAIN", 20.0)  # Max 20dB gain
    print("         - Target: 0.08 RMS, Max Gain: 20dB")
    ...
    print("    - Conservative AGC (0.08 RMS, 20dB max) for far-field pickup")
    ...
    print("  This amplifies distant speech, then removes the 601Hz electrical tone.")
    print("  Stationary NS learns noise in first ~2-3 seconds, then subtracts it.\n")
    
    # Save configuration state for listener
    config_dict = {
        'HPFONOFF': 1,
        'AGCONOFF': 1,  # Enabled - conservative
        'AGCDESIREDLEVEL': 0.08,
        'AGCMAXGAIN': 20.0,
...
def main():
    ...
    try:
        ...
        if preset == "far_field":
            ...
            print(f"\n  Available presets:")
            print(f"    - clean      : HPF + Conservative AGC + Max Stationary NS")
