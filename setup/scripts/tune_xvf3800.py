...
def configure_agc_20_ec():
    ...
    print("  - AGC: ENABLED (target: 0.15 RMS, max gain: 20x linear = ~26 dB, response: 0.3s)")
    ...
    run_xvf_command("PP_AGCTIME", 0.3)  # Faster response (0.3s) for better clipping prevention - hardware-only solution
    ...
    save_config_state('agc_20_ec', {
        ...
        'PP_AGCTIME': 0.3,  # Faster response for hardware-only clipping prevention
        'PP_ECHOONOFF': 1
    })
    return True
