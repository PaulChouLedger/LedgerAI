...
This directory contains all RAG (Retrieval-Augmented Generation) related components for the LLM Generic Container.
...
- **Purpose**: Monitor `data/input/` for new document files
- **Features**:
...
1. **Source**: Documents (text files, PDFs, JSON, etc.) in `data/input/`
2. **Text Extraction**: Documents converted to text format
...
2. **Change Detection**: New/modified document files detected
...
- **Container REST API**: Uses RAG for document-based Q&A
- **Generic LLM**: Provides context-aware responses via RAG
