...
4. **Audio Normalization**: Normalized to optimal RMS (0.22) for Whisper
...
**Function**: `normalize_audio_for_whisper(audio_data, target_rms=0.22)`
...
1. **`balanced_beam`** ⭐ RECOMMENDED
   ...
   - AGC: Enabled (target: 0.22 RMS, max gain: 4.0x linear = ~12.0 dB)
...
- `PP_AGCDESIREDLEVEL`: Target RMS level (0.22 = recommended)
...
3. **Normalization**: Adjusts to optimal RMS (0.22) for Whisper
...
1. **Audio Processing**:
   ...
   - Normalization: Same as main transcription (target RMS: 0.22)
...
- Check audio normalization: Ensure RMS ~0.22
