...
class DebugOverlayWidget(QWidget):
    ...
    def __init__(self, parent, window_size):
        ...
        self.max_lines = 0  # Show last 12 lines of debug messages
...
class AuraGUI(QMainWindow):
    ...
    def create_circular_buttons(self):
        ...
        button_configs = [
            ("↑", "Upload",    self._handle_upload,    "#B4B4B4"),  # Mercury  - metallic grey
            ("⚙", "Settings",  self._handle_settings,  "#E6D2A0"),  # Venus    - pale yellow
            ("📊", "Analytics",self._handle_analytics, "#4696C8"),  # Earth    - blue-green
            ("🎤", "Voice",    self._handle_voice,     "#C85A46"),  # Mars     - rusty red
            ("📱", "Mobile",   self._handle_mobile,    "#D2A06E"),  # Jupiter  - warm tan
            ("ℹ", "Info",      self._handle_info,      "#325AB4"),  # Neptune  - deep blue
        ]
        ...
        radius = 420  # Distance from center to button (further from edge to avoid border interference)
        center_x = 540  # Center of 1080x1080 screen
        center_y = 540
        
        self.buttons = []

        button_size = 150  # <<< make sure this line exists
        ...
        for i, (text, tooltip, handler, color) in enumerate(button_configs):
            ...
            btn.setFixedSize(button_size, button_size)  # e.g. 150x150, set above
