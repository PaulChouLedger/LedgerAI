...
def speak_llm_response(prompt):
    import requests
    print(f"[RAG] ✅ Sending prompt to RAG system: {prompt}")

    try:
        # Try RAG first, fallback to direct LLM
        response = requests.post(
            "http://localhost:5003/rag",
            json={"query": prompt, "top_k": 3},
            stream=True,
            timeout=60
        )

        # RAG returns JSON response, not streaming
        result = response.json()
        response_text = result.get("response", "")
        
        # Split response into sentences for TTS
        from chat import split_into_sentences
        sentences = split_into_sentences(response_text)
        for sentence in sentences:
            enqueue_tts_chunk(sentence)

    except Exception as e:
        print(f"[RAG] ❌ RAG failed, falling back to direct LLM: {e}")
        ...
        try:
            ...
            print(f"[LLM] ❌ Direct LLM also failed: {e2}")
