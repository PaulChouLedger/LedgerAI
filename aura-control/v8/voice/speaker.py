...
# Per-style TTS tuning (exaggeration, cfg_weight/temperature overrides)
STYLE_PARAMS = {
    "neutral":    dict(exaggeration=0.05, cfg_weight=0.25),
    "warm":       dict(exaggeration=0.10, cfg_weight=0.35),
    "empathy":    dict(exaggeration=0.12, cfg_weight=0.35),
    "assertive":  dict(exaggeration=0.08, cfg_weight=0.30),
    "technical":  dict(exaggeration=0.03, cfg_weight=0.20),
    "playful":    dict(exaggeration=0.14, cfg_weight=0.40),
    "energy":     dict(exaggeration=0.18, cfg_weight=0.45),
    "soft":       dict(exaggeration=0.06, cfg_weight=0.25),
}
...
def _generate_tts_audio(text: str, style: str = "neutral"):
    ...
    base_ex = sp.get("exaggeration", 0.05)
    base_cfg = sp.get("cfg_weight", 0.25)
    base_temp = float(os.getenv("CHATTERBOX_BASE_TEMPERATURE", "0.78"))
    base_top_p = float(os.getenv("CHATTERBOX_BASE_TOP_P", "0.92"))
    base_pace = float(os.getenv("CHATTERBOX_BASE_PACE", "1.02"))
    ...
    kwargs = {}
    # Turbo variant ignores cfg_weight, min_p, exaggeration — skip to avoid warnings
    if not _chatterbox_is_turbo:
...
class Speaker:
    ...
    # Cycle through voice styles for LLM responses to use different ref samples
    _LLM_STYLES = ["neutral", "warm", "empathy", "soft", "energy", "assertive"]
