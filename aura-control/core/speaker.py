...
# === Output smoothing / protection for tiny puck speakers ===
# How many audio chunks to buffer before starting playback (jitter buffer)
# Higher = more stable playback, slightly more latency
TTS_JITTER_PREBUFFER_CHUNKS = int(os.getenv("TTS_JITTER_PREBUFFER_CHUNKS", "12"))

# Simple digital soft limiter to prevent clipping into small speakers
TTS_LIMITER_ENABLED = os.getenv("TTS_LIMITER_ENABLED", "1") == "1"
# Peak threshold in int16 space (max is 32767). 30000 is ~-1 dBFS.
TTS_LIMITER_THRESHOLD = int(os.getenv("TTS_LIMITER_THRESHOLD", "30000"))


def _soft_limit_chunk(chunk: bytes) -> bytes:
    """
    Apply a simple peak limiter to a 16-bit mono PCM chunk.
    If samples exceed TTS_LIMITER_THRESHOLD, the whole chunk is scaled down.
    """
    if not TTS_LIMITER_ENABLED or not chunk:
        return chunk
    ...
    try:
        ...
        if peak == 0 or peak <= TTS_LIMITER_THRESHOLD:
            return chunk

        # Compute safe gain and apply in float then back to int16
        gain = (TTS_LIMITER_THRESHOLD * 0.95) / float(peak)
        audio = (audio.astype(np.float32) * gain).astype(np.int16)
        return audio.tobytes()
    except Exception as e:
        print(f"[Speaker] ⚠️ Soft limiter failed: {e}")
        return chunk
...
def tts_playback_thread(text, tts_start_time):
    """
    Stream TTS audio from the selected engine into aplay with:
    - jitter prebuffer (to avoid underruns / crackles)
    - soft limiter (to reduce harshness on tiny puck speakers)
    """
    with playback_lock:
        set_playing(True)

        # Track approximate TTS usage based on text length
        try:
            from wallet.wallet_integration import get_usage_tracker
            tracker = get_usage_tracker()
            words = len(text.split())
            speech_duration_seconds = words / 2.0 if words else 0.0  # ~2.5 words/s
            tracker.record_usage("tts_generation", multiplier=speech_duration_seconds)
        except Exception as e:
            print(f"[TokenUsage] ⚠️ Failed to track TTS usage: {e}")

        try:
            # 1) Ask the TTS engine for a streaming generator of PCM chunks
            audio_gen = _generate_tts_audio(text)

            # 2) Jitter buffer: pre-fill a few chunks before starting aplay
            prebuffer = []
            total_bytes = 0
            ...
            for chunk in audio_gen:
                if not chunk:
                    continue
                chunk = _soft_limit_chunk(chunk)
                prebuffer.append(chunk)
                total_bytes += len(chunk)

                if len(prebuffer) >= TTS_JITTER_PREBUFFER_CHUNKS:
                    break

            if not prebuffer:
                print("[Speaker] ⚠️ No audio produced by TTS engine.")
                return

            # 3) Select ALSA output device (your UACDemo puck output if present)
            if OUTPUT_CARD_INDEX is not None:
                ...
                print("[Speaker] 🔊 Playing TTS on default ALSA device")

            proc = subprocess.Popen(
                ["aplay", "-D", alsa_device, "-f", "S16_LE",
                 "-r", str(PCM_SAMPLE_RATE), "-c", "1"],
                stdin=subprocess.PIPE,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            )

            # 4) Log perceived latency from LLM finish → first audio out
            try:
                tts_latency = time.time() - tts_start_time
                print(f"[Speaker] ⏱️ TTS latency (LLM → audio start): {tts_latency:.2f}s")
            except Exception:
                pass

            # 5) Feed the prebuffered audio first (smooth start)
            for chunk in prebuffer:
                ...
                try:
                    ...
                    print("[Speaker] ⚠️ aplay terminated during prebuffer write")
                    break

            # 6) Stream the rest of the generator
            for chunk in audio_gen:
                if not chunk:
                    continue
                chunk = _soft_limit_chunk(chunk)
                try:
                    ...
                    # aplay died; no point continuing
                    if proc.poll() is not None:
                        print(f"[Speaker] ⚠️ aplay exited with code {proc.returncode}")
                    else:
                        print("[Speaker] ⚠️ aplay BrokenPipe with unknown status")
                    break

            # 7) Clean shutdown of aplay
            try:
                proc.stdin.close()
            except Exception:
                pass
            ...
            print(f"[Speaker] ❌ TTS playback error: {e}")
            import traceback
            traceback.print_exc()
        finally:
            set_playing(False)
