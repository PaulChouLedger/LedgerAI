...
SPEECH_FLATNESS_MAX = 0.25      # Balanced threshold (speech ~0.01-0.2, noise ~0.5-1.0) - allows legitimate speech with slight noise
...
def is_likely_speech(features, duration=None):
    ...
    # Only reject if flatness is clearly in noise range AND other features suggest noise
    if features['spectral_flatness'] > 0.30:  # Clearly in noise range (not borderline)
        if features['rms'] < 0.030 and duration is not None and duration < 0.5:
