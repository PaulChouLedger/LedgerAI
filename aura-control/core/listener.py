...
def is_likely_speech(features, duration=None):
    ...
    # Combined check: Very high flatness + low energy + short duration = likely noise
    # Only reject if flatness is clearly in noise range (0.5+) AND other features suggest noise
    if features['spectral_flatness'] > 0.50:  # Clearly in noise range (not borderline speech)
