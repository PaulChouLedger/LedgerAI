...
def test_semantic_keyword_approach():
    print("🧪 Testing Semantic + Keyword Similarity Approach")
    print("=" * 70)
    ...
    try:
        ...
        # Test cases for semantic + keyword approach
        test_cases = [
            # Should be HIGH similarity with keyword boost
            {
                ...
                "expected": "HIGH (Diverticulitis - should get keyword boost for 'left', 'lower')"
            },
            {
                "user_response": "left lower quadrant pain",
                "guideline_location": "LEFT LOWER QUADRANT (LLQ) - key differentiator from appendicitis (RLQ). LOCALIZED and CONSTANT.",
                "expected": "HIGH (Diverticulitis - should get strong keyword boost for 'left', 'lower', 'quadrant')"
            },
            {
                "user_response": "upper right side under my ribs",
                "guideline_location": "RIGHT UPPER QUADRANT (RUQ) pain with CVA tenderness.",
                "expected": "HIGH (Cholecystitis - should get keyword boost for 'right', 'upper')"
            },
            {
            ...
            # Should be LOW similarity (different locations, no keyword boost)
            {
                "user_response": "left lower belly pain towards my pelvis",
                "guideline_location": "EPIGASTRIC, midline upper abdomen. No radiation typically.",
                "expected": "LOW (Peptic Ulcer - wrong location, no keyword overlap)"
            },
            {
                "user_response": "upper right side under my ribs",
                "guideline_location": "LEFT LOWER QUADRANT (LLQ) - key differentiator from appendicitis (RLQ).",
                "expected": "LOW (Diverticulitis - wrong location, no keyword overlap)"
            },
            ...
            # Edge cases
            {
                ...
                "guideline_location": "LEFT LOWER QUADRANT (LLQ) - key differentiator from appendicitis (RLQ).",
                "expected": "MEDIUM (generic vs specific - minimal keyword overlap)"
            },
            {
                "user_response": "abdominal pain",
                "guideline_location": "EPIGASTRIC, midline upper abdomen. No radiation typically.",
                "expected": "MEDIUM (generic vs specific - some keyword overlap with 'abdomen')"
            },
            ...
            # Complex descriptions
            {
                ...
                "guideline_location": "LEFT LOWER QUADRANT (LLQ) - key differentiator from appendicitis (RLQ). LOCALIZED and CONSTANT.",
                "expected": "HIGH (Complex Diverticulitis - should get keyword boost for 'left', 'lower')"
            },
            {
                "user_response": "dull ache in my upper right side just below my ribcage",
                "guideline_location": "RIGHT UPPER QUADRANT (RUQ) pain with CVA tenderness.",
                "expected": "HIGH (Complex Cholecystitis - should get keyword boost for 'right', 'upper')"
            }
        ]
        ...
        for i, case in enumerate(test_cases, 1):
            ...
            try:
                # Test semantic + keyword approach
                enhanced_similarity = engine._compute_enhanced_location_similarity(
                    case['user_response'], 
                    case['guideline_location']
                )
                print(f"Semantic + Keyword similarity: {enhanced_similarity:.3f}")
                
                # Test pure semantic for comparison
                pure_semantic = engine._compute_similarity(
                    case['user_response'], 
                    case['guideline_location']
                )
                print(f"Pure semantic similarity: {pure_semantic:.3f}")
                
                # Show improvement
                improvement = enhanced_similarity - pure_semantic
                if improvement > 0:
                    print(f"📈 Keyword boost: +{improvement:.3f}")
                
                # Categorize scores
                if "HIGH" in case['expected']:
                    high_scores.append(enhanced_similarity)
                    print(f"🎯 HIGH similarity expected")
                elif "LOW" in case['expected']:
                    low_scores.append(enhanced_similarity)
                    print(f"📉 LOW similarity expected")
                else:
                    medium_scores.append(enhanced_similarity)
        ...
        print("\n🎯 SUMMARY:")
        print("Semantic + Keyword approach should:")
        print("✅ Provide semantic understanding of medical terminology")
        print("✅ Boost scores for exact keyword matches")
        print("✅ Better differentiate between similar locations")
        print("✅ Work for any medical condition")
        print("✅ Handle complex descriptions naturally")
...
if __name__ == "__main__":
    test_semantic_keyword_approach()
