...
app = Flask(__name__)
# Load .env file from LedgerAI root (mounted at /app/.env in container)
# This file is generated by aura_config.sh
load_dotenv("/app/.env")
...
# === Model Config ===
# Read from .env file (generated by aura_config.sh)
# Handle both absolute and relative paths
_model_path = os.getenv("SIMPLE_MODEL_PATH", "/models/Llama-3.2-1B.Q4_K_M.gguf")
if not os.path.isabs(_model_path):
    # Convert relative path to absolute (models are in /models/)
    if _model_path.startswith("models/"):
        _model_path = _model_path.replace("models/", "", 1)
    SIMPLE_MODEL_PATH = f"/models/{_model_path}"
else:
    SIMPLE_MODEL_PATH = _model_path
