...
class ProactiveAnalyzer:
    ...
    def __init__(self, memory_manager: MemoryManager, 
        ...
        self.min_conversations_for_analysis = 5  # Need at least 5 conversations to analyze (increased for more context)
    ...
    def analyze_and_suggest(self, current_conversation: str) -> Optional[str]:
        ...
        """
        logger.debug(f"🔍 Starting analysis for: '{current_conversation[:80]}...'")
        ...
        if time_since_last < self.suggestion_cooldown:
            ...
            logger.debug(f"⏳ Cooldown active ({remaining:.1f}s remaining), skipping analysis")
            return None
        
        # Need minimum conversations to analyze
        total_conversations = len(self.memory_manager.conversations)
        if total_conversations < self.min_conversations_for_analysis:
            logger.debug(f"ℹ️ Not enough conversations ({total_conversations} < {self.min_conversations_for_analysis}), skipping analysis")
            return None
        
        logger.debug(f"📊 Total conversations: {total_conversations}, proceeding with analysis")
        
        # Search for similar conversations
        logger.debug(f"🔎 Searching for similar conversations (threshold: {self.similarity_threshold})...")
        ...
        if not similar:
            logger.debug("ℹ️ No similar conversations found above threshold")
            return None
        ...
        logger.debug(f"📚 Retrieved {len(recent)} recent conversations for context")
        
        # Analyze patterns and generate suggestion
        logger.debug("🧠 Generating suggestion using LLM...")
        ...
        if suggestion:
            ...
            logger.info(f"💡 Suggestion generated successfully")
        else:
            logger.debug("ℹ️ No suggestion generated by LLM")
